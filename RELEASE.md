...
`namespace-release/vX.Y.Z` branch (for example, `namespace-release/v1.109.5`) that contains:
...
upstream release. The `.release` file records the exact upstream version currently targeted by `main`.
...
Resolve and commit any conflicts without discarding Namespace changes. Apply or update Namespace patches, set the release target,
then run the appropriate install, audit, build, and test checks:

```bash
./scripts/set-release-version.sh "$VERSION"
```

Commit the updated source and `.release` together. Once reviewed, publish the release-ready `main`:
...
Run the helper with the upstream version. It verifies `main` and creates `namespace-release/v1.109.5` at the exact tested commit:

```bash
./scripts/prepare-release-branch.sh "$VERSION"
git push --set-upstream origin "namespace-release/v${VERSION}"
```
...
```bash
gh workflow run .github/workflows/openvscode-server-release.yml \
  --repo namespacelabs/openvscode-server \
  --ref main \
  -f version="$VERSION" \
  -f revision="$REVISION" \
  -f quality=stable \
...
A release is a build of a `namespace-release/vX.Y.Z` branch in this repository, shipped under a Namespace release tag. Prepare and validate
...
- `version` — the upstream OpenVSCode Server version, e.g. `1.109.5`. The workflow derives branch `namespace-release/v1.109.5` and upstream
...
branch does not match the `namespace-release/vX.Y.Z` name derived from the requested version, if `.release` or `package.json` does not match
that version, or if the upstream tag is not an ancestor of the fork commit.
...
git fetch upstream --tags
git merge --no-ff "openvscode-server-v${VERSION}"
# Resolve and commit conflicts before continuing.
./scripts/set-release-version.sh "$VERSION"
```
...
The branch script verifies `.release` and the versions in upstream and local `package.json`, checks that the working tree is clean,
ensures the upstream tag is in the history, and creates `namespace-release/v1.109.5` from the current `main` commit when needed. If the branch
already exists, it switches to and validates that branch without moving it. The script changes only the local repository. Review
the result and publish the branch explicitly. It fails if a local release branch is stale or has diverged from its remote, requiring
you to reconcile the histories explicitly before releasing:
